...
date: "`r doc_date()`"
vignette: >
  %\VignetteIndexEntry{Multi-sample multi-group spatially resolved transcriptomic data}
...
After downloading the raw data from the original source, we merge samples across different time phases, perform quality control to filter low-quality genes and cells, and apply @Banksy for multi-sample clustering and smoothing. The finalized SPE objects are made available via Bioconductor's ExperimentHub for easy access and reproducibility.
...
# If you want to download the full data (about 2.1 GB in RAM) use:
if (benchmarkme::get_ram() > 4e9) {
    Wei22_full()
}
```
