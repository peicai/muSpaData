...
Since manual annotations are unavailable in the original dataset, we used Banksy [@Banksy] to define spatial domains by jointly modeling multiple samples. The Banksy spatial cluster assignments are available in the `colData()`.

```{r view ARTISTA Banksy, fig.width=5,fig.height=4}
# View LIBD layers for one sample
CD <- colData(spe_example) |> as.data.frame()
ggplot2::ggplot(CD, 
    aes(x=sdimx,y=sdimy, 
